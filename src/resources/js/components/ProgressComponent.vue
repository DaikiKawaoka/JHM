    ...
    <div v-if="students">
      <header class="header_container">
        ...
        <div class="progress_view_box">
          <select class="progress_view_select" v-model="ProgressViewSelected" v-on:change="selectProgressView">
          ...
          <form action="/progress/excel_export" method="get">
            <input id="excel_dl_btn" type="submit" class="btn btn-success btn-lg mb-1" value="Excelダウンロード" />
          </form>
        </div>
      </header>
      <div class="progress-page-all" style="overflow-y: scroll">
        <table class="table table-bordered" :style="'width:' + table_width_px + 'px;'" >
          <thead>
            <tr>
              <th class="fixed" rowspan="4" style="
                  width: 65px;
                  text-align: center;
              ...
              <th class="fixed2" rowspan="4" style="
                  width: 100px;
                  text-align: center;
                  vertical-align: middle;
                  padding: 0;
              ...
              <th v-for="entry_count in most_many_entry_num" :key="entry_count" :colspan="max_progress_count" style="width: 400px; text-align: center; padding: 0">
                応募先企業名
              </th>
            </tr>
            <tr style="width: 100px">
              <template v-for="entry_count in most_many_entry_num" :key="entry_count">
                <th v-for="index in max_progress_count" :key="index" class="header-progress" style="text-align: center; padding: 0">
                  選考{{ index }}
                </th>
              </template>
            </tr>
            <tr style="width: {{entry_column_width_px}}px;">
              <th v-for="entry_count in most_many_entry_num" :key="entry_count" :colspan="max_progress_count" style="text-align: center; padding: 0">
                日付
              </th>
            </tr>
            <tr style="width: {{entry_column_width_px}}px;">
          ...
          <tbody>
            <template v-for="(student, s_index) in students" :key="student">
              <tr>
                <td class="fixed" rowspan="4" style="
                    width: 65px;
                    text-align: center;
                    vertical-align: middle;
                    padding: 0;
                ...
                <td class="fixed2" rowspan="4" style="
                    width: 100px;
                    text-align: center;
                    vertical-align: middle;
                    padding: 0;
                ...
                <template v-for="(entry, e_index) in entries_list[s_index]" :key="(entry.id, e_index)">
                  <!-- 求人からエントリーした会社 -->
                  <td v-if="entry.company_name != null" class="entered-company-name" :colspan="max_progress_count" style="text-align: center; padding: 0">
                    {{ entry.company_name }}
                  </td>
                  <!-- 生徒自身が登録した会社 -->
                  <td v-else class="entered-student-company-name" :colspan="max_progress_count" style="text-align: center; padding: 0">
                    {{ entry.student_company_name }}
                  </td>
                </template>
                <!-- 空のエントリー要素を作成 -->
                <!-- エントリー数が1番多い生徒ではない場合 (表を綺麗に見せるため) -->
                <template v-if="entries_list[s_index].length < most_many_entry_num">
                  <!-- エントリー数が1番多い人との差分だけfor分で回す -->
                  <template v-for="i in most_many_entry_num - entries_list[s_index].length" :key="i">
              ...
              <tr>
                ...
                <template v-for="(entry, e_index) in entries_list[s_index]" :key="(entry.id, e_index)">
                  ...
                  <template class="student-progress-list" v-for="progress in progress_list[s_index][e_index]" :key="progress.id">
                    <td style="width: 100px; text-align: center; padding: 0">
                      {{ progress.action }}
                    </td>
                  </template>
                  <template v-if="progress_list[s_index][e_index].length < max_progress_count">
                    <template class="progress-list" v-for="j in max_progress_count - progress_list[s_index][e_index].length" :key="j">
                      <td style="width: 100px; text-align: center; padding: 0">
                        &nbsp;
                      </td>
                    </template>
                ...
                <template v-if="entries_list[s_index].length < most_many_entry_num">
                  ...
                  <template v-for="i in most_many_entry_num - entries_list[s_index].length" :key="i">
                    ...
                    <template class="progress-list" v-for="i in max_progress_count" :key="i">
                      <td style="width: 100px; text-align: center; padding: 0">
                        &nbsp;
                      </td>
                    </template>
